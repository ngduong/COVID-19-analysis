...
distancing_df = distancing_df %>% 
  mutate(
    percent_infected = cases / population
  )

highlights = unlist(data = c(distancing_df[14, ], distancing_df[286, ], distancing_df[214, ]))

distancing_df %>% 
  ggplot(aes(x = t, y = cases)) +
  geom_path(aes(color = region)) +
  geom_point(data = distancing_df[14,]) +
  geom_point(data = distancing_df[286,]) +
  geom_point(data = distancing_df[130,]) +
  labs(
...
ggsave("./visualizations/kmeans_graph.jpg", width = 12, height = 8)
```
...
ggsave("./visualizations/gmm_graph.jpg", width = 12, height = 8)
```
