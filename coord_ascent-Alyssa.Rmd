...
country_reg = country_reg[1:10]
a.seq = seq(0, 1e6, 1e4)
b.seq = seq(0, 1, 0.01)
c.seq = seq(0, 90, 1)
param_df = NULL
fitted_list <- replicate(length(country_reg), data.frame())

for (i in 1:length(country_reg)){
 param = fit_curve(by_country %>% filter(country_region == country_reg[i]), 
                      aseq = a.seq, bseq = b.seq, cseq = c.seq)
 fitted = logistic_pred(as.numeric(as.matrix(filter(by_country,country_region == country_reg[i])[,2])),
