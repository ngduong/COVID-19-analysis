...
Plotting Data
```{r}
list.names = names(fitted_list)

lns = sapply(fitted_list, nrow)
fitted_list = as.data.frame(do.call("rbind", fitted_list))
fitted_list$group = rep(list.names, lns)

ggplot(fitted_list, aes(x = x, y = y, colour = group)) +
    theme_bw() +
    geom_line(linetype = "dotted")

plot(unlist(fitted_list), type = "n", xlim = c(1, max(sapply(fitted_list, length))), main = "Number of Cumulative COVID-19 Cases for All Countries", xlab = "Days Since First Case", ylab = "Number of Cases")
mapply(lines, fitted_list, col = seq_along(fitted_list))
...
next_countries = c("United Kingdom", "France", "Italy", "Tanzania", "South Africa", "Myanmar", "Kenya", "South Korea", "Colombia", "Spain", "Uganda", "Argentina", "Algeria", "Sudan", "Ukraine", "Iraq")
...
ggsave("top_20_regions.jpg", width = 12, height = 8)
```
